...
ROS2 Humble(`ros-humble-ros-base` + `cv_bridge` + `launch_ros`)을 설치해서 `colcon build`,
`ros2 run`, `ros2 launch`(YAML 파라미터 로딩 포함)까지 실제로 동작 확인했다 (2026-08-17).
단, **로봇/카메라/Nav2 스택 자체가 아직 없어서** 실제 이미지 토픽을 받아 VPS 서버에
쿼리하는 end-to-end 흐름은 검증 못 했다 — 노드가 뜨고 파라미터를 읽고 구독/퍼블리셔를
만드는 것까지만 확인된 상태.
...
- `calibration_translation` / `calibration_quaternion`: 아래 "map 프레임 캘리브레이션" 참고
...
이건 로봇의 `map` 프레임(occupancy grid 원점)과 대개 다르다.
...
2. 그 로봇 pose(`map` 프레임 기준 translation+quaternion)를 기록해둔다 — 이게
   `calibration_translation`/`calibration_quaternion`이 된다. (hloc world 원점 = 스캔
   시작 시점의 카메라 pose = 이 로봇 pose라고 보고 T_map_from_world = 그 로봇 pose로 둔다.)
3. `config/vps_localizer.yaml`에 그 값을 채워 넣는다.

이 저장소엔 아직 이 절차를 자동화하는 도구가 없다 (로봇/스캔 장비가 없어 검증 불가) —
위 수동 절차대로 1회성으로 값을 채워 넣는 방식이다.
