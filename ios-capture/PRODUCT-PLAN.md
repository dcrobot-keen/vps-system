...
- [x] **CI 테스트 게이트 승격 완료(2026-09-04)** — 사용자가 맥에서 만든 `ScanMeshTests` 타깃(pbxproj 변경)을 푸시, `ios.yml`의 `xcodebuild test`에서 `continue-on-error` 제거 → 정식 필수 게이트. (사소한 뒷정리 거리: 첫 시도 때 생긴 빈 `vpsTests` 타깃이 pbxproj에 같이 남아있음 — 소스 파일이 없어 CI엔 무해하지만, Xcode에서 그 타깃만 지워도 됨. 급하지 않음.)
