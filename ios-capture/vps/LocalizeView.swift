    ...
    private var topDownView: some View {
        Canvas { context, size in
            ...
            let bounds = TopDownBounds(
                trajectory: manager.trajectory,
                current: displayPose.map { SIMD2($0.x, $0.y) },
                background: manager.floorPlanBackground
            )
            let mapPoint: (SIMD2<Double>) -> CGPoint = { bounds.project($0, into: size) }

            if let background = manager.floorPlanBackground {
                let topLeft = mapPoint(SIMD2(background.minX, background.maxY))
                let bottomRight = mapPoint(SIMD2(background.maxX, background.minY))
                let rect = CGRect(
                    x: topLeft.x, y: topLeft.y,
                    width: bottomRight.x - topLeft.x, height: bottomRight.y - topLeft.y
                )
                context.opacity = 0.7
                context.draw(Image(uiImage: background.image), in: rect)
                context.opacity = 1
            }
...
/// 궤적 점들 + 현재 위치(+있으면 floorplan.png 배경 지도)를 감싸는 정사각형 범위를
/// 잡아 Canvas 크기에 맞춰 투영한다. 화면 좌표는 y가 아래로 증가하므로 world y는
/// 뒤집어서 그린다(+y가 화면 위쪽이 되도록).
private struct TopDownBounds {
    let centerX: Double
    let centerY: Double
    let span: Double

    /// background가 있으면 그 전체가 항상 보이도록 범위에 포함시킨다 -- "지금 스캔한
    /// 방 전체 지도 위에 내 위치가 어디 있는지"가 궤적 몇 점만 보고 계속 확대/축소가
    /// 바뀌는 것보다 안정적으로 낫다.
    init(trajectory: [SIMD2<Double>], current: SIMD2<Double>?, background: FloorPlanBackground?) {
        var points = trajectory
        if let current { points.append(current) }
        if let background {
            points.append(SIMD2(background.minX, background.minY))
            points.append(SIMD2(background.maxX, background.maxY))
        }
