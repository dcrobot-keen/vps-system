...
>
> **번들 ID 변경(2026-09-06): `com.dcrobot.scanmesh` → `com.dcrobot.keen.scanmesh`.**
> TestFlight 업로드 중 App Store Connect에 같은 이름으로 삭제된 앱 레코드가 있었는데
> 자가 복원(제거된 앱 목록에서도)이 안 돼서, 테스트 타깃들이 이미 쓰던
> `com.dcrobot.keen.*` 패턴에 맞춰 새 ID로 교체(미등록 상태라 충돌 없음). 코드 내
> `os.Logger` subsystem, 백그라운드 URLSession 식별자도 함께 변경.
...
**5단계(프로젝트 단위 위치 확인, 2026-09-04):** "위치 확인도 합쳐서 할 수 있나"에
대한 답 — 지도(ARWorldMap)는 세션당 하나만 로드되고 합치는 API가 없어서 **지도를
순서대로 시도 + 정렬 변환으로 좌표 옮기기**로 구현. `LocalizeSessionManager`가
후보(`LocalizeCandidate` = 스캔 + `ScanAlignment`) 목록을 받아, 사용자가 "지금 근처
스캔"을 고르면 그 지도만, 안 고르면 스캔 1부터 18초 타임아웃마다 다음 지도로 자동
순환(ARKit이 실패를 알려주지 않음). 잡히면 그 스캔의 정렬 변환
(`ScanAlignment.applyGroundPose`/`applyXZ`/`rotateXZ`)으로 기준 스캔 좌표계로 옮겨
표시 — 정확도는 수동 정렬만큼. 상단 2D 배경은 정렬 화면과 같은 공용 코드
(`FloorPlanLayer.swift`: 레이어/범위/매핑/그리기)로 각 스캔 바닥 평면을 변환 적용해
겹쳐 그리고 궤적도 변환해 깔아준다. **방향 관례 통일**: 위치 확인 화면이 예전엔
GroundPose(y = -z) 기준으로 위 = -z였는데(이미지를 뒤집어 그림), 정렬 화면·floorplan.png와
상하가 반대라 **위 = +z**로 맞췄다(이미지 뒤집기 제거, 화살표/궤적도 world x/z로
직접 그림). 프로젝트 ⋯ 메뉴에 "위치 확인 (서버 없이)"(지도 있는 스캔이 하나라도
있을 때). 스캔 단위 위치 확인은 후보 하나(identity)로 같은 코드를 탄다.
  - **정정(2026-09-04, 실기 보고)**: 위 = +z는 **거울상**이었다 — 왼쪽으로 돌면 화면
    화살표는 오른쪽으로 돌았다. ARKit world는 +y가 위인 오른손 좌표계라 실제로 위에서
    내려다보면 +x가 오른쪽일 때 **+z는 아래**(위 = -z, 카메라 시작 방향이 위)다. 근본
    원인은 `FloorPlanRenderer`가 floorplan.png를 row 0 = 최대 z로 그린 것(그 이미지를
    기준으로 관례를 "통일"하면서 거울상이 전파됨). 고친 내용: 렌더러가 row 0 = 최소
    z로 그리고 floorplan.json에 `format_version: 2`를 쓴다; 옛 스캔(버전 키 없음)은
    `PersistedMeta.load`가 읽을 때 png를 세로로 뒤집고 origin_top_z를 다시 계산해
    v2로 고쳐 쓴다(프로세스 전체 lock으로 이중 뒤집기 방지, "바닥 평면 보기"용으로
    ProjectDetailView가 화면에 들어올 때 미리 한 번 읽음); 공용 2D 매핑
    (`FloorPlanLayer`)은 위 = -z, 이미지 회전은 -yaw; 정렬 화면의 드래그(z 부호)/
    두 손가락 회전(부호)/±5° 버튼(아이콘 방향)도 같이 바꿈. 저장된 `ScanAlignment`
    값 자체(world 좌표 변환)는 그대로 유효하다.
- [ ] 실기 검증 안 됨 — 자동 순환 타임아웃 체감, 지도 교체 시 트래킹 재시작이
  거슬리는지, 정렬 변환 적용된 위치가 다른 스캔 영역에서 실제로 맞는지, 그리고 위 =
  -z로 바꾼 뒤 화살표/배경이 실제 움직임과 일치하는지(왼쪽으로 돌면 화살표도 왼쪽).

**정렬 화면 개선 + 자동 맞춤(2026-09-04, "합치기가 잘 안 되고 UI도 어렵다" 보고):**
원인은 정렬 도구가 실제로 맞출 수 있게 돼 있지 않았던 것 — floorplan.png가 못 본
곳까지 회색으로 꽉 찬 불투명 사각형이라 고른 스캔이 기준 스캔을 덮어 겹침을 볼 수
없었고, 회전 축이 스캔 원점(화면에 안 보이는 스캔 시작 지점)이라 두 손가락으로
돌리면 이미지가 멀리 날아갔으며, 드래그와 회전이 동시에 걸려 흔들리고, 확대가 안 됐다.
- `FloorPlanPixels`(신규): png를 RGBA로 풀어 픽셀 분류(검정=벽, 205 회색=미확인,
  나머지=바닥/기타). 미확인만 투명하게 뚫은 `overlayImage`(두 화면 공용 `FloorPlanLayer.image`),
  역할별 틴트 이미지(기준 흰색·고른 스캔 주황·나머지 회색 벽, 바닥 옅게, 경로/마커/사진
  바닥은 원래 색), 벽 픽셀 중심의 world (x, z) 점(`wallPointsXZ`, 최대 6000).
- `ScanAlignment.inverseXZ`/`rotated(by:aboutX:z:)`: pivot 고정 회전. 정렬 화면의 두 손가락
  회전과 ±5° 버튼이 이미지 중심을 축으로 돈다.
- `ScanAlignmentView`: 고른 스캔 위에서 시작한 드래그는 스캔 이동, 빈 곳 드래그는 화면
  팬, 핀치 줌(`TopDownMapping.zoomed`), 두 손가락 제스처가 시작되면 같이 발동한 드래그는
  되돌리고 끝날 때까지 무시, "전체 보기" 버튼으로 범위 재설정.
- **자동 맞춤**: `ScanRegistration`(신규) = scan-to-map-studio `studio/registration.py`의
  2D ICP(icp_2d / multistart)를 Swift로 이식(cKDTree 대신 격자 해시, 2D라 SVD 없이
  atan2 닫힌 식 Kabsch). 손으로 놓은 자리를 초기값으로 yaw ±12° 5개 시드 → 거친(대응
  0.5m) → 정밀(대응 0.15m) ICP, 대응 비율 30% 미만이면 결과를 버리고 "손으로 더 가깝게
  맞춘 뒤 다시" 안내. target은 고른 스캔을 뺀 나머지 스캔 전부(현재 정렬 적용)라 3개
  이상도 순서대로 붙일 수 있다. 원본과 같은 한계(초기값 ±30° 안, 대칭 방/겹침 부족에
  약함) 때문에 "완전 자동"이 아니라 "마무리"로 붙였다.
- 실기 검증 안 됨. 남은 것: 합친 mesh 미리보기에서 스캔별 색 구분과 실제 합쳐진
  스캔 수 표시(usdz 로드 실패가 조용히 빠짐), 바닥 높이 맞춤을 최솟값 대신 중앙값으로.

**실기 결과(2026-09-04 저녁) → "이전 스캔 위치에 맞추기"(ARKit anchoring):** 자동 맞춤이
틀리게 붙었다. `data/scan_20260904_2104xx` 3개를 데스크탑 파이썬으로 분석한 결과 (1)
floorplan.png의 "벽"은 선이 아니라 분류 삼각형을 채운 덩어리라 점 대응이 안 되고, (2) 무엇보다
**세 스캔이 서로 다른 방**이라 겹치는 구조가 없어 어떤 기하 정합도 원리적으로 불가능했다
(회전 전수 탐색 + 상관 + trimmed ICP까지 해봐도 대응 40~60%에 모순 25~30% — 우연히 벽 모양이
겹친 자리). 그래서:
- **`ScanSessionManager.startAnchoring`**: 새 스캔 시작 전에 프로젝트의 이전 스캔 지도
  (worldmap)로 재국지화(최근 스캔부터, 18초 타임아웃 순환 — LocalizeSessionManager와 같은
  방식)하고, 잡히는 순간의 카메라 pose로 "새 세션 좌표계 → 옛 지도 좌표계" 변환
  (`ScanAlignment.fromCameraTransform`: 새 세션 원점 = 그 순간 기기 위치, -z = 기기 전방의
  수평 성분)을 만들어 후보의 정렬 변환과 합성(`ScanAlignment.then`)한 뒤 **곧바로
  resetTracking으로 독립 세션 캡처**를 시작한다. 예전 "이어서 스캔"(옛 지도를 계속 씀)과
  달리 재국지화는 몇 초만 쓰고 스캔 자체는 지금처럼 새 세션이다. `ScanView`에 토글
  ("이전 스캔 위치에 맞추기", 기본 켬) + 맞추는 중 패널("맞추지 않고 시작"/"취소"),
  저장 시 `onSaved(ScanAlignment?)`로 넘겨 `ScanGroupStore.setAlignment`에 기록.
  전제: 각 방 스캔이 문 밖 복도를 1~2m 같이 담고, 다음 방은 그 문 근처에서 시작(안내 문구
  추가). 안 잡히면 수동 정렬.
- 자동 맞춤은 "같은 공간을 겹쳐 찍은 경우"용으로만 남기고 입력/판정을 고침: 벽 점을
  scan.usdz mesh의 바닥 위 1.0~1.6m 높이 띠 단면(`ScanRegistration.wallSliceXZ`, 실기
  데이터에서 벽이 선으로 또렷)으로 바꾸고(usdz 없으면 floorplan 벽 픽셀), 상대 스캔이
  바닥으로 본 자리에 벽이 놓이는 비율(`conflictFraction`, `FloorPlanLayer.freePointsXZ`)을
  재서 대응 60% 미만이거나 모순 12% 초과면 결과를 버리고 "다른 방이면 손으로 놓거나
  이전 스캔 위치에 맞추기로 찍으라"고 안내한다.
- 실기 검증 안 됨. 특히 `fromCameraTransform`의 "새 세션 -z = 기기 전방 수평 성분" 가정
