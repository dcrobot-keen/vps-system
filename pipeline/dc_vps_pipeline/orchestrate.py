...
- `--scan-to-map-studio-dir`를 생략하면 hloc DB + `base_map.ply`까지만 만든다 (VPS 전용).
- 준다면 https://github.com/dcrobot-keen/scan-to-map-studio 의 `scripts/studio.py new` /
  `process --ply`를 그대로 subprocess로 호출한다 (그 프로젝트는 자체 venv를 쓰므로
...
def orchestrate(
    ...
    print(f"[2/3] 포인트클라우드 export -> {pointcloud_path}")
